...
class YAMLProvider implements ProviderTemplate
{
    ...
    {
        return new Config(MoneyAPI::getInstance()->getDataFolder() . "money.yml", Config::YAML);
    }
    ...
    public function addMoney(string $xuid, float $money): void
    {
        $config = $this->getDataBase();
        $secure = MoneyAPI::getInstance()->getSecure()->get($xuid);
        $ev = new PlayerUpdateMoneyEvent($secure["pseudo"], $xuid, $this->getMoney($xuid), $this->getMoney($xuid) + $money);
        $ev->call();
